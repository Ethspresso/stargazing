...
function setup() {
    ...
    let arc_count_idx = floor(fxrand() * arc_counts.length);
    arc_count = arc_counts[arc_count_idx];

    let paletteNames = Object.keys(palettes);
    palette = paletteNames[floor(fxrand() * paletteNames.length)];
    ...
    star_points = map(star_ray_len, 0, 0.25, 128, 512);
    let star_size = "Small";
    ...
    const canvas = {};
    canvas.w = innerWidth;
    canvas.h = innerHeight;
    console.log('Rendering at', canvas.w, 'x', canvas.h);
...
function draw_bg() {
    ...
    let bg_point_count = floor(width * height * 0.0025);

    // Draw each background star
    for (let i = 0; i < bg_point_count; i++) {
        let x = fxrand(width);
        let y = fxrand(height);
        if (i < bg_point_count * 0.9) stroke(0, 0, fxrand(10, 70), 0.9);
        else stroke(240, 90, fxrand(25, 70), 0.9);
        strokeWeight(2);
        point(x, y);
        point_count++;
    }
...
function draw_arcs(iter) {
    ...
    for (let i = 0; i < arc_count; i++) {
        for (let t = start_t; t < end_t; t += 1 / arc_point_count) {
            let x = bezierPoint(-len * start_off, len, len, len * end_x_off, t);
            let y = bezierPoint(-len * start_off, 0, len, len * end_y_off, t);

            if (x < innerWidth && y < innerHeight) {
                // Fade colors based on distance to center
                let fade = 1.2 * map(dist(0, 0, x, y), 0, width * 1.1, 1, 0);
                stroke(hue, 80, 100 * fade, 0.9);

                // Perturb the location a bit, based on distance to center
                let amplitude = 1.2 * map(dist(0, 0, x, y), 0, width * 1.1, 2, 8);
...
function draw_star() {
    ...
    for (let i = 0; i < star_repeat; i++) {
        ...
        for (let t = 0; t < 1; t += 1 / star_points) {
            let x = bezierPoint(-len * 0.01, 0, 0, len, t);
            let y = bezierPoint(-len * 0.01, 0, 0, len, t);

            // Fade colors based on distance to center
            let fade = map(dist(0, 0, x, y), 0, width / 2, 1, 0.4);
            stroke(hue, 90, 90 * fade, 0.6);
...
function draw() {
    if (arcs) draw_arcs(frameCount);

    if ((arcs && frameCount >= max_iter) || (!arcs && frameCount === 1) || early_stop) {
