...
function setup() {
  ...
  palette = paletteNames[Math.floor($fx.rand() * paletteNames.length)];
  ...
  arc_count = arc_counts[Math.floor($fx.rand() * arc_counts.length)];
  ...
  star = $fx.rand() < 0.95 ? true : false;
  let star_repeat_opts = [256, 512, 1024];
  star_repeat = star_repeat_opts[Math.floor($fx.rand() * star_repeat_opts.length)];
  let star_ray_len_opts = [0, 0.05, 0.1, 0.15, 0.2, 0.25];
  star_ray_len = star_ray_len_opts[Math.floor($fx.rand() * star_ray_len_opts.length)];
  let star_point_count_min = 600 * (buffer_width/1000);
  let star_point_count_max = 6000 * (buffer_width/1000);
  star_point_count = map(star_ray_len, 0, 0.25, star_point_count_min, star_point_count_max);
  ...
  end_x_off = map($fx.rand(), 0, 1, -3, 5);
  end_y_off = map($fx.rand(), 0, 1, 2, 5);
  ...
  switch (star_size) {
    case 'Small':
      start_off = map($fx.rand(), 0, 1, 0.03, 0.1);
      break;
    case 'Medium':
      start_off = map($fx.rand(), 0, 1, 0.1, 0.2);
      break;
    case 'Big':
      start_off = map($fx.rand(), 0, 1, 0.2, 0.3);
      break;
    case 'Massive':
      start_off = map($fx.rand(), 0, 1, 0.3, 0.4);
      break;
    default:
      // 'No star' variants can be any size
      start_off = map($fx.rand(), 0, 1, 0.03, 0.4);
      break;
  }
  ...
  buffer2 = createGraphics(buffer_width, buffer_height, WEBGL);
  ...
  buffer2.colorMode(HSB, 360, 100, 100, 1.0);
...
function draw_bg(b) {
  ...
  for (let i = 0; i < bg_point_count; i++) {
    let x = $fx.rand() * b.width;
    let y = $fx.rand() * b.height;
    if (i < bg_point_count * 0.9) b.stroke(0, 0, map($fx.rand(), 0, 1, 35, 70), 0.9);
    else b.stroke(240, 90, map($fx.rand(), 0, 1, 50, 100), 0.9);
    b.strokeWeight(2);
    b.point(x, y);
    point_count++;
  }
  ...
  for (let i = 0; i < arc_count; i++) {
    for (let t = start_t; t < end_t; t += 1 / arc_point_count) {
      ...
      if (x < b.width && y < b.height) {
        ...
        let amplitude = map(b.dist(x, y, 0, 0), 0, b.width * 1.2, 1, max_point_amplitude);
        let off = amplitude * map($fx.rand(), 0, 1, -1, 1);
        b.point(x + off, y - off);
        point_count++;
      } else {
        early_stop = true;
...
function draw_star(b, iter) {
  ...
  let end_t = map(iter, 0, max_star_iter, 0, 1);
  ...
  for (let i = 0; i < star_repeat; i++) {
    ...
    let len = b.width * 0.2 + map($fx.rand(), 0, 1, -b.width * 0.05, b.width * star_ray_len);
    ...
    // Fade colors based on distance to center
    let fade = map(b.dist(0, 0, x2, y2), 0, b.width / 2, 1, 0.4);
...
function draw_star_body(b) {
  ...
  star_size_px = map(start_off, 0.03, 0.4, star_size_px_min, star_size_px_max);
...
function draw() {
  ...
  let progress = Math.round(map(frameCount, 1, max_star_iter, 10, 100));
  if (debug) show_buffer(buffer, buffer2, width, height);
  else show_progress(progress);

  if (star) draw_star(buffer, frameCount);
  if (frameCount > max_star_iter) {
    draw_arcs(buffer2, frameCount - max_star_iter);
    show_buffer(buffer, buffer2, width, height);
  }

  if (frameCount >= (max_arch_iter+max_star_iter) || early_stop) {
...
function show_buffer(b1, b2, w, h) {
  ...
  push();
  background('black');
  image(b1, 0, 0, w, h, 0, 0, b1.width, b1.height, CONTAIN);
  image(b2, 0, 0, w, h, 0, 0, b2.width, b2.height, CONTAIN);
  pop();
}
...
function show_progress(amount) {
  ...
  textFont('Verdana');
  textAlign(CENTER);
  textSize(Math.floor(42*window.innerHeight/1000));
  fill('white');
  stroke('white');
  text('Stargazing by Ethspresso', innerWidth/2, innerHeight/2);
  textSize(Math.floor(32*window.innerHeight/1000));
  text(amount + '%', innerWidth/2, innerHeight/2 + Math.floor(64*window.innerHeight/1000))
}
